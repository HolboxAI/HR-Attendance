...
export function MonthCalendar({
  ...
  return (
    <div className="bx-card p-4">
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-semibold uppercase tracking-widest text-ink-3">
        ...
        {days.map((d) => {
          ...
          return (
            <div
              key={d.date}
              title={label}
              className={`relative flex aspect-square flex-col items-center justify-center rounded-md border text-sm ${
                d.date === today ? 'border-accent' : 'border-transparent'
              } ${future ? 'opacity-30' : off ? 'opacity-50' : 'bg-surface-2/60'}`}
            >
              <span className="sr-only">{label}</span>
              <span aria-hidden className={`text-xs ${future ? 'text-ink-3' : statusText(d.status)}`}>
                {future ? '·' : statusGlyph(d.status)}
              </span>
              <span aria-hidden className="tnum text-xs text-ink-2">{Number(d.date.slice(8))}</span>
              {d.has_exception && !future && (
                <span aria-hidden className="absolute right-1 top-1 size-1.5 rounded-full bg-st-late" />
              )}
            </div>
          );
        })}
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-ink-3">
        ...
        <span><span aria-hidden className="text-st-late">•</span> corner dot = needs attention</span>
      </div>
    </div>
  );
}
