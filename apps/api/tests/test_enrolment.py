...
def punch(code):
    ...
    return client.post(
        ...
        data={"employee_code": code, **AT_OFFICE},
    ).json()
...
r = client.get("/api/v1/admin/enrolments").json()
...
r = client.post(
    ...
    data={"employee_code": "TX002"},
)
check("accepted", r.status_code, 200)
img = client.get("/api/v1/admin/enrolments/TX002/photo")
...
check("list-upload", client.post(
    ...
    data={"employee_code": "NOPE"}).status_code, 404)
check("photo", client.get("/api/v1/admin/enrolments/NOPE/photo").status_code, 404)
check("delete", client.delete("/api/v1/admin/enrolments/NOPE").status_code, 404)

print("19. Withdrawing through the API")
check("delete ok", client.delete("/api/v1/admin/enrolments/TX002").status_code, 200)
db.expire_all()
check("bob unenrolled", active_enrolment(db, bob) is None, True)
r = client.get("/api/v1/admin/enrolments").json()
check("summary shows the gap", r["summary"]["missing"], 1)
check("delete again is 404", client.delete("/api/v1/admin/enrolments/TX002").status_code, 404)
