...
/**
 * Check in from the dashboard - roadmap item 4, and the demo of the whole
 * punch pipeline without needing the phone app.
 *
 * Everything here goes through the REAL endpoints. The camera frame is posted
 * to /mobile/punch exactly as a handset would post its selfie: same geofence,
 ...
 * signed in.
 *
 * The install-id field is visible on purpose. On a phone it is invisible
...
/** Org timezone, never the raw ISO string - a rule the whole dashboard follows. */
...
export default function CheckinPage() {
  ...
  // green "camera in use" light stays on after the punch - which reads as
  // spyware, not attendance.
  ...
  async function openCamera() {
    ...
    try {
      ...
      setPhase('camera');
      // Next tick, so the <video> exists before it gets a stream.
  ...
  async function punch() {
    ...
    // No direction field: the server decides in/out from the day so far,
    ...
    <div className="mx-auto max-w-xl space-y-6">
      <div>
        ...
        <p className="mt-1 max-w-prose text-sm text-ink-2">
          The same pipeline as the phone: camera, location, device binding, then a
          face comparison against your enrolled photo. Whoever is signed in is who
          punches — there is no way to punch as someone else.
        </p>
      </div>
      ...
      {todayError ? (
        ...
        <div className="bx-card flex flex-wrap items-baseline gap-x-4 gap-y-1 p-4 text-sm">
          ...
          <span className="text-ink-2">
            {today.checked_in_at
              ? `in at ${at(today.checked_in_at)}${today.checked_out_at ? `, out at ${at(today.checked_out_at)}` : ''}`
              : 'not checked in yet'}
          </span>
        </div>
      ) : (
