...
/**
 * The mock identity, for the same reason the rest of api.ts has a mock: the
 * whole screen flow has to be demonstrable before a server exists. It is
 * clearly labelled and cannot be reached when USE_MOCK is false, so it is not
 * a back door - flipping that one constant is what makes this app real.
 */
const MOCK_IDENTITY: Identity = {
  user_id: 'mock-user',
  email: 'demo@boxcode.ai',
  role: 'employee',
  employee_id: 'mock-employee',
  employee_code: 'BX000',
  full_name: 'Demo (mock mode)',
  can_punch: true,
  is_admin: false,
};
...
export async function signIn(email: string, password: string): Promise<SignInResult> {
  if (USE_MOCK) {
    ...
    const identity = { ...MOCK_IDENTITY, email: email.trim().toLowerCase() };
    await saveSession({ access: 'mock', refresh: 'mock', identity });
...
export async function restore(): Promise<Identity | null> {
  ...
  if (USE_MOCK) return session.identity;
