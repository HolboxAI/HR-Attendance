...
const MAP: Record<string, { label: string; dot: string; text: string; glyph: string }> = {
  ...
  wfh:        { label: 'WFH',       dot: 'bg-cyan-500',   text: 'text-cyan-500 font-medium', glyph: '🏠' },
...
export function Status({ value, size = 'sm', isRegularized = false }: { value: string; size?: 'sm' | 'xs'; isRegularized?: boolean }) {
  const s = MAP[value] ?? MAP.not_marked;
  const label = isRegularized && value === 'present' ? `${s.label} — Corrected` : s.label;
  return (
    <span className="inline-flex items-center gap-2 whitespace-nowrap">
      <span className={`h-1.5 w-1.5 rounded-full ${s.dot}`} aria-hidden />
      <span className={`${size === 'xs' ? 'text-xs' : 'text-sm'} ${s.text}`}>{label}</span>
    </span>
  );
}
