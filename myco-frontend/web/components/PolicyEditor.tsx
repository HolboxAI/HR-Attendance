...
export function PolicyEditor({
  ...
  return (
    <div className="space-y-10">
      {error && (
        <p role="alert" className="text-xs text-st-absent font-mono">
          {error}
        </p>
      )}
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 max-w-prose text-xs text-ink-3 font-mono">
            These apply to everyone. Changing them takes effect immediately for new requests; nothing already approved is recalculated.
          </p>
        </div>

        <div className="grid gap-4 rounded-2xl glass-panel border border-line p-6 sm:grid-cols-3">
          ...
          <label className="space-y-1.5">
            ...
            />
            <span className="block text-[10px] text-ink-3 font-mono">
          ...
          <label className="space-y-1.5">
            ...
            <span className="flex items-center gap-2 py-2 cursor-pointer">
              <input
                id="sandwich" type="checkbox" checked={pol.sandwich_rule}
                onChange={(e) => setPol({ ...pol, sandwich_rule: e.target.checked })}
                className="size-4 rounded border-line"
              />
              <label htmlFor="sandwich" className="text-xs font-semibold text-ink cursor-pointer">
                {pol.sandwich_rule ? 'Enabled' : 'Disabled'}
              </label>
            </span>
            <span className="block text-[10px] text-ink-3 font-mono">
        ...
        <div className="flex items-center gap-3">
          ...
          {saved === 'policy' && (
            <span className="text-xs text-st-present font-mono">Saved</span>
          )}
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-4">
          {rows.map((t) => (
            <div key={t.id} className="rounded-2xl glass-panel border border-line p-6">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <h3 className="font-display text-lg font-bold text-ink">
                  {t.name} <span className="text-xs font-mono text-ink-3">{t.code}</span>
                </h3>
                {!t.is_paid && (
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <label className="space-y-1.5">
                  ...
                  <span className="flex items-center gap-2 py-2 cursor-pointer">
                    <input
                      ...
                      className="size-4 rounded border-line"
                    />
                    <label htmlFor={`cf-${t.id}`} className="text-xs font-semibold text-ink cursor-pointer">
                ...
                <label className="space-y-1.5">
                  ...
                  <input
                    ...
                    className={`${field} tnum w-full disabled:opacity-40 font-mono`}
                  />
                </label>
              </div>

              <div className="mt-5 flex flex-wrap items-center gap-4 border-t border-line/60 pt-4">
                <label className="flex items-center gap-2 text-xs font-medium text-ink-2 cursor-pointer">
                  <input
                ...
                <div className="ml-auto flex items-center gap-3">
                  {saved === t.id && (
                    <span className="text-xs text-st-present font-mono">
                      Saved — balances unchanged
                    </span>
                  )}
                  <button
                    type="button" onClick={() => saveType(t)} disabled={busy === t.id}
                    className="rounded-xl bg-ink text-ground px-4 py-2 text-xs font-black uppercase tracking-wider disabled:opacity-50 hover:opacity-90 active:scale-95 transition-all cursor-pointer"
                  >
